...
uint8_t NRF24L01_check(void)
{
	...
	for(j = 0; j < CHIP_CHECK_TIMES || !CHIP_CHECK_TIMES; j++)//未达到最大重复次数或为无穷次
	{
...
{
    uint8_t addr[5] = {RF_INIT_ADDR};//地址设置
    ...
    NRF24L01_Write_Reg( FEATRUE, 0x04 );//(原代码为0x07)
    ...
    NRF24L01_Write_Reg( RF_CH, RF_INIT_CH );             	//初始化通道(原代码为60)
    NRF24L01_Write_Reg(RF_SETUP, (RF_INIT_DR << 3) | RF_INIT_PWR);//配置(原代码为0x26)
...
void NRF24L01_TxStart(uint8_t *txbuf, uint8_t len)//启动发送
{
	...
	RF24L01_SET_CE_LOW( );
	NRF24L01_Write_Buf(WR_TX_PLOAD, txbuf, len);	//写数据到TX BUF 32字节  TX_PLOAD_WIDTH
	RF24L01_SET_CE_HIGH( );			//启动发送
}
...
uint8_t NRF24L01_TxCheck(void)//检查发送
{
	...
	if(RF24L01_GET_IRQ_STATUS( ) != 0){	//若还未发送成功
		TxCount++;
		if(TxCount > CHECK_MAX_TIMES_TX){	//超过最大次数仍未发送成功
			TxCount = 0;	//计数清零
			return 0xFE;	//返回超时标志
		}
...
uint8_t NRF24L01_RxPkg(uint8_t *rxbuf)
{
	...
	if(RF24L01_GET_IRQ_STATUS( ) != 0){	//还未接收到
		RxCount++;
		if(RxCount >= CHECK_MAX_TIMES_RX){	//超过最大次数仍未收到数据
			RxCount = 0;	//计数清零
			return 0xFE;	//返回超时标志
		}
		return 0xFD;	//返回正在接收标志
	}
	RxCount = 0;	//计数清零
  ...
  * @retval:无
  */
void NRF24L01_Config(uint8_t ifRX)
{
	NRF24L01_Gpio_Init( );//RF24L01控制引脚初始化
	G24_Chip_State |= 0x40;
	if(NRF24L01_check()){//检测到nRF24L01
		G24_Chip_State |= 0x80;//标记芯片在位
		RF24L01_Init( );//RF24L01初始化
		if(ifRX){//若为接收模式
			RF24L01_Set_Mode( MODE_RX );//RF24L01设为接收模式
			if(ifRX == 2) NRF24L01_RxStart();//启动接收
		}
	}
}
